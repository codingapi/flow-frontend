...
flow-pc 是 Flow Engine 前端的 PC 端组件库集合，包含基础 UI 组件、表单组件和审批组件，基于 Ant Design 组件库构建。
...
PC 端基础 UI 组件库，提供原子化组件（如按钮、输入框等），基于 Ant Design 封装。

### flow-pc-form

PC 端表单组件库，提供表单设计和表单渲染能力：
...
PC 端审批组件库，提供完整的审批流程界面：

#### 布局组件

```typescript
// packages/flow-pc/flow-pc-approval/src/components/flow-approval/layout/header.tsx
...
// packages/flow-pc/flow-pc-approval/src/components/flow-approval/layout/body.tsx
export const ApprovalBody: React.FC<{
    content: FlowContent;
    review?: boolean;
}> = ({ content, review }) => { ... }

// packages/flow-pc/flow-pc-approval/src/components/flow-approval/layout/footer.tsx
...
// packages/flow-pc/flow-pc-approval/src/components/flow-approval/components/action/reject.tsx
export const RejectAction: React.FC = () => { ... }

// packages/flow-pc/flow-pc-approval/src/components/flow-approval/components/action/delegate.tsx
export const DelegateAction: React.FC = () => { ... }

// packages/flow-pc/flow-pc-approval/src/components/flow-approval/components/action/transfer.tsx
export const TransferAction: React.FC = () => { ... }

// packages/flow-pc/flow-pc-approval/src/components/flow-approval/components/action/add-audit.tsx
export const AddAuditAction: React.FC = () => { ... }

// packages/flow-pc/flow-pc-approval/src/components/flow-approval/components/action/return.tsx
export const ReturnAction: React.FC = () => { ... }

// packages/flow-pc/flow-pc-approval/src/components/flow-approval/components/action/save.tsx
export const SaveAction: React.FC = () => { ... }

// packages/flow-pc/flow-pc-approval/src/components/flow-approval/components/action/revoke.tsx
export const RevokeAction: React.FC = () => { ... }

// packages/flow-pc/flow-pc-approval/src/components/flow-approval/components/action/urge.tsx
export const UrgeAction: React.FC = () => { ... }

// packages/flow-pc/flow-pc-approval/src/components/flow-approval/components/action/custom.tsx
export const CustomAction: React.FC<{ action: FlowAction }> = ({ action }) => { ... }

// packages/flow-pc/flow-pc-approval/src/components/flow-approval/components/action/close.tsx
export const CloseAction: React.FC<{ onClose?: () => void }> = ({ onClose }) => { ... }
```

#### 默认视图实现

PC 端为审批视图插件提供了基于 Ant Design 的默认实现：

```typescript
// packages/flow-pc/flow-pc-approval/src/plugins/view/add-audit-view.tsx
export const AddAuditView: React.FC<AddAuditViewPlugin> = (props) => {
    // 使用 ViewBindPlugin 获取已注册的自定义视图
    // 如果没有注册，则使用默认的 Select 组件
}

// packages/flow-pc/flow-pc-approval/src/plugins/view/delegate-view.tsx
export const DelegateView: React.FC<DelegateViewPlugin> = (props) => { ... }

// packages/flow-pc/flow-pc-approval/src/plugins/view/transfer-view.tsx
export const TransferView: React.FC<TransferViewPlugin> = (props) => { ... }

// packages/flow-pc/flow-pc-approval/src/plugins/view/return-view.tsx
export const ReturnView: React.FC<ReturnViewPlugin> = (props) => { ... }

// packages/flow-pc/flow-pc-approval/src/plugins/view/sign-key-view.tsx
...
// packages/flow-pc/flow-pc-approval/src/plugins/view/manual-view.tsx
...
```typescript
// packages/flow-pc/flow-pc-approval/src/components/flow-approval/typings/plugin-type.ts
...
export const APPROVAL_BODY_SIDER_VIEW_KEY = 'APPROVAL_BODY_SIDER_VIEW_KEY';
