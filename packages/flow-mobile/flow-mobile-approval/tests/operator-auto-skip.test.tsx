...
const createOperator = (overrides: Partial<FlowApprovalOperator> = {}): FlowApprovalOperator => ({
    ...
    actionName: '',
    actionType: '',
    flowOperator: {userId: 1, name: '李四', flowManager: false},
    ...overrides,
});

describe.sequential('移动端审批记录操作人展示（自动跳过）', () => {
    ...
    test('autoSkip 记录展示为自动跳过，不展示审批时间与动作', () => {
        ...
        expect(screen.getByText('自动跳过: 李四')).toBeTruthy();
    ...
    test('autoSkip 缺省（旧数据）时按实际审批人展示', () => {
        ...
        expect(screen.getByText(/审批人: 李四/)).toBeTruthy();
